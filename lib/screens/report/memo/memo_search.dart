...
class _MSearchState extends State<MSearch> {
  final List<String> _options = ['Memo No', 'Company Name', 'Bill No', 'Date'];
  String _selectedOption = 'Bill No';
  List<Memo> _foundRes = [];

  void _runFilter(String enteredKeyword) {
    final allUsers = Provider.of<Report>(context, listen: false).mdetails;
    setState(() {
      ...
      } else if (_selectedOption == 'Memo No') {
        _foundRes = allUsers.where((user) =>
            user.memoNo.toLowerCase().contains(enteredKeyword.toLowerCase())).toList();
      } else if (_selectedOption == 'Date') {
        _foundRes = allUsers.where((user) =>
            user.date.toLowerCase().contains(enteredKeyword.toLowerCase())).toList();
      }else {
        _foundRes = allUsers.where((user) =>
            user.billNo.toLowerCase().contains(enteredKeyword.toLowerCase())).toList();
      }
    });
  }
                  ...
                  return GestureDetector(
                    ...
                    child: Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text('MN:'+_foundRes[i].memoNo.toString()),
                        trailing: GestureDetector(
                          onTap: () {
                            Clipboard.setData(ClipboardData(text: _foundRes[i].billNo));
